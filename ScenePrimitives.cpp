...
namespace
{
	...
	generate_box_vertices( glm::vec3 position, float width, float height, float depth, float repeat )
	{
		...
		return {
			{ { position.x - width / 2.f, position.y + height / 2.f, position.z + depth / 2.f }, color, { 0.0f, repeat } },
			{ { position.x + width / 2.f, position.y + height / 2.f, position.z + depth / 2.f }, color, { repeat, repeat } },
			{ { position.x + width / 2.f, position.y - height / 2.f, position.z + depth / 2.f }, color, { repeat, 0.0f } },
			{ { position.x + width / 2.f, position.y - height / 2.f, position.z + depth / 2.f }, color, { repeat, 0.0f } },
			{ { position.x - width / 2.f, position.y - height / 2.f, position.z + depth / 2.f }, color, { 0.0f, 0.0f } },
			{ { position.x - width / 2.f, position.y + height / 2.f, position.z + depth / 2.f }, color, { 0.0f, repeat } },
